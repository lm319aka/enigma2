...
class _E2Async(_E2):
    """
    Low-level Asynchronous Enigma2 Cipher class.
    
    This class inherits from the low-level `_E2` class and provides asynchronous interfaces
    ...
    Key behaviors:
      1. Uses `_E2Params` (permitting odd/imperfect btypes).
      2. Bypasses data compression during encryption (but decodes compression if detected
         during decryption of metadata-wrapped files).
    """

    def __init__(self, params: _E2Params):
        """
        Initialize the low-level _E2Async cipher instance.

        :param params: An instance of _E2Params containing operational parameters.
                       Can use odd/imperfect btypes.
        """
    ...
    async def encrypt_async(self, 
        ...
        for integrity check during decryption, running the core encryption loop
    ...
    async def decrypt_async(self, 
        ...
        This method parses the metadata header, verifies the HMAC-SHA256 signature,
        performs the core decryption loop on a worker thread, and returns the validated
        original plaintext.
